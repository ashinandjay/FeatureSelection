...
def zCurve(x):
    t=[]
    A = x.count('A'); C = x.count('C'); G = x.count('G');TU=x.count('T')
    x_ = (A + G) - (C + TU)
    y_ = (A + C) - (G + TU)
    z_ = (A + TU) - (C + G)
...
if(option == "1"):
    ...
    #Pseknc method
    vector=Pseknc(fastas,lamada=3, w=0.05, k = 2)
    vector.to_csv("Pseknc_out.csv")
elif(option == "5"):
    #DAC method
    ...
    #TAC method
    vector=TAC(fastas,k=2,lag=1)
    vector.to_csv("TAC_out.csv")
elif(option == "9"):
    #TCC method
    vector=TCC(fastas,k=2,lag=1)
    vector.to_csv("TCC_out.csv")
elif(option == "10"):
    #TACC method
    vector=TACC(fastas,k=2,lag=1)
    vector.to_csv("TACC_out.csv")
elif(option == "11"):
    #NAC method
    vector=NAC(fastas)
